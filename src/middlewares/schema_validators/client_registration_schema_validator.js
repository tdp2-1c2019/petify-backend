...
function ClientRegistrationSchemaValidator(logger) {
  let _logger = logger;

  this.validateRequest = function(req, res, next) {
    _logger.debug('Request received: %j', req.body);
    let validator = new SchemaValidator();
    validator.validateJson(req.body, 'client_registration_request.json', function(err) {
      if (err) {
...
module.exports = ClientRegistrationSchemaValidator;
